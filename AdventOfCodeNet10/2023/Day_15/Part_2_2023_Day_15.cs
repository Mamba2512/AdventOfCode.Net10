...
namespace AdventOfCodeNet10._2023.Day_15
{
    using System.Diagnostics;
    using LensConfig = (string LensLabel, int FocalLength);
  internal class Part_2_2023_Day_15 : Days
  {
    /// <summary>
    /*
    ...
    */
    ...
    /// </returns>
    /// 
    Dictionary<int, List<LensConfig>> Boxes = new Dictionary<int, List<LensConfig>>();
    public override string Execute()
    {
      ...
      //   with the content of the Test-Example-Input_2023_Day_15.txt already stored in "Lines"
      //
      // Second -> THE REAL TEST !! <-
      // with the content of the Input_2023_Day_15.txt already stored in "Lines"
      //
      foreach (var line in Lines)
      {
        ...
        foreach (var chunk in chunks)
        {
          ...
          bool containsEqualSign = chunk.Contains('=');
          GetLensConfiguration(chunk, containsEqualSign);
          //Debug.Write($"After {chunk}: ");
          //Debug.WriteLine("");
        ...
        //      {
        //        Debug.Write($"Box{kvp.Key}: ");
        //        foreach(var val in kvp.Value)
        //        {
        //          Debug.Write($"[{val.LensLabel}{val.FocalLength}] ");
        //        }
        //        Debug.WriteLine("");
        //      }
        //    }
        ...
        foreach(var kvp in Boxes)
        {
          if (kvp.Value != null)
          {
            if (kvp.Value.Count > 0)
            {
              ...
              {
                var first = boxIdx + 1;
                var second = lensIdx + 1;
                var third = val.FocalLength;
                totalCount += (first * second * third);
                lensIdx++;
                //Debug.Write($"[{val.LensLabel}{val.FocalLength}] ");
              }
              //Debug.WriteLine("");
    ...
    private void GetLensConfiguration(string currentValue, bool isEqualSign)
    {
      if (isEqualSign)
      {
        ...
        LensConfig currentLensConfig = (label, focalLength);
        var boxIndex = GetBoxIndex(label);
        ModifyBoxWhenEqualSign(boxIndex, currentLensConfig);
      }
      else
      {
        var parts = currentValue.Split('-', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
        var label = parts[0];
        var boxIndex = GetBoxIndex(label);
        ModifyBoxWhenMinusSign(boxIndex, label);
      }
    }
    ...
    private bool BoxContainsLensLabel(int boxIndex, LensConfig currentConfig)
    {
      return Boxes[boxIndex].Any(lc => lc.LensLabel == currentConfig.LensLabel);
    }

    private bool BoxContainsLensLabel(int boxIndex, string label)
    {
      return Boxes[boxIndex].Any(lc => lc.LensLabel == label);
    }

    private void ModifyBoxWhenEqualSign(int boxIndex, LensConfig lensConfig)
    {
      var containsLensLabel = BoxContainsLensLabel(boxIndex, lensConfig);
      if(containsLensLabel)
      {
        //var lensIndex = Boxes[boxIndex].IndexOf(lensConfig);
        var listOfLensConfigInCurrentBox = Boxes[boxIndex];
        int lensIndex = 0;
        foreach(var elt in listOfLensConfigInCurrentBox)
        {
          if(elt.LensLabel == lensConfig.LensLabel)
          {
            lensIndex = listOfLensConfigInCurrentBox.IndexOf(elt);
            break;
          }
        }
        listOfLensConfigInCurrentBox[lensIndex] = lensConfig;
        Boxes[boxIndex] = listOfLensConfigInCurrentBox;
      }
      else
      {
        var numberOfLensConfigForCurrentBox = Boxes[boxIndex].Count();
        var listOfLensConfigInCurrentBox = Boxes[boxIndex];
        listOfLensConfigInCurrentBox.Add(lensConfig);
        Boxes[boxIndex] = listOfLensConfigInCurrentBox;
      }
    }

    private void ModifyBoxWhenMinusSign(int boxIndex, string label)
    {
      var containsLensLabel = BoxContainsLensLabel(boxIndex, label);
      if(containsLensLabel)
      {
        var listOfLensConfigInCurrentBox = Boxes[boxIndex];
        listOfLensConfigInCurrentBox.RemoveAll(lc => lc.LensLabel == label);
        Boxes[boxIndex] = listOfLensConfigInCurrentBox;
      }
    }
  }
