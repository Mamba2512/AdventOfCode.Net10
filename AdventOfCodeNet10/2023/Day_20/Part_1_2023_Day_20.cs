namespace AdventOfCodeNet10._2023.Day_20
{
  using FlipFlop = (string Name, bool IsOn, string InputSignal, string OutputSignal, List<string> Destinations);
  using Conjunction = (string Name, Dictionary<string, string> InputSignals, string OutputSignal, List<string> Destinations);
  using Broadcast = (string Name, string InputSignal, string OutputSignal, List<string> Destinations);

  internal class Part_1_2023_Day_20 : Days
  {
    /// <summary>
    /*
    ...
    */
    ...
    /// </returns>
    /// 
    List<(string, List<string>)> input = new();
    List<Broadcast> broadcasters = new();
    List<Conjunction> conjunctions = new();
    List<FlipFlop> flipFlops = new();
    ...
    public override string Execute()
    {
      ...
      //   with the content of the Test-Example-Input_2023_Day_20.txt already stored in "Lines"
      //
      // Second -> THE REAL TEST !! <-
      // with the content of the Input_2023_Day_20.txt already stored in "Lines"
      //
      foreach (var line in Lines)
      {
        var splitted = line.Split("->", StringSplitOptions.TrimEntries);
        var moduleName = splitted[0];
        var destinations = splitted[1].Split(',', StringSplitOptions.TrimEntries).ToList();
        input.Add((moduleName, destinations));
      }
    ...
    private void SingleButtonPress()
    {
      // Send a low pulse to the broadcaster
      var pulseQueue = new Queue<(string ModuleName, string PulseType)>();
      pulseQueue.Enqueue(("broadcaster", "low"));

      while(pulseQueue.Count > 0)
      {
        var pulse = pulseQueue.Dequeue();
        var moduleName = pulse.ModuleName;
        var pulseType = pulse.PulseType;
        // Find the module
    ...
    private void ProcessAllInputs()
    {
      ...
      {
        var module = elt.Item1;
        var destinations = elt.Item2;
        if(module == "broadcaster")
        {
          // Broadcast
          broadcasters.Add((module, "", "", destinations));

        }
        else if (module.StartsWith("&"))
        {
          // Conjunction
          conjunctions.Add((module, new Dictionary<string, string>(), "", destinations));
        }
        else if (module.StartsWith("%"))
        {
          // FlipFlop
          flipFlops.Add((module, false, "", "", destinations));
        }
      }
    }
  }
